...
const HeroCard = () => {
  return (
    <div className="relative mx-auto flex min-h-[760px] w-full max-w-7xl flex-col items-center justify-center gap-12 px-5 pb-16 pt-48 sm:px-10 md:min-h-[860px] lg:flex-row lg:items-center lg:justify-between lg:gap-12 lg:px-8 lg:pt-56 lg:pb-32 xl:gap-16">
      {/* Left — Text Content with lots of breathing room */}
      <div className="z-10 flex w-full max-w-xl flex-col items-center text-center lg:items-start lg:text-left gap-5 sm:gap-6 lg:flex-1">
        ...
        {/* CTA */}
        <div className="mt-5 flex flex-wrap items-center justify-center gap-4 sm:mt-8 sm:gap-6 lg:justify-start">
          <a
            href="#booking"
      ...
      <div className="relative flex w-full shrink-0 justify-center lg:w-auto">
        ...
        <div className="glass relative w-full max-w-[360px] sm:max-w-[420px] md:max-w-[440px] lg:max-w-[480px] rounded-[2rem] bg-white/70 p-3 shadow-2xl sm:p-4 md:rounded-[2.5rem]">
          {/* Subtle gradient overlay to blend perfectly */}
          <div className="pointer-events-none absolute inset-4 z-10 rounded-[1.5rem] sm:rounded-4xl bg-linear-to-tr from-black/20 to-transparent mix-blend-overlay"></div>
          <img
            src={premiumTajMahal}
            alt="Taj Mahal — Just 40km from Firozabad"
            className="relative z-0 aspect-[0.86] h-auto w-full rounded-[1.5rem] object-cover sm:rounded-4xl"
          />
          
          {/* Small floating badge */}
          <div className="glass absolute -bottom-5 -left-2 sm:-left-8 z-20 flex items-center gap-3 rounded-2xl bg-white/90 px-4 py-3 shadow-xl transition-transform hover:-translate-y-1 sm:gap-4 sm:rounded-3xl sm:px-6 sm:py-4">
